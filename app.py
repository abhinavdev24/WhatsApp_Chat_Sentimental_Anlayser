...
if uploaded_file is not None:
    ...
    st.title("WhatsApp Chat Sentimental Analysis for " + selected_user)
    if st.sidebar.button("Show Analysis"):

        # getting the stats of the selected user from the stats script

        num_messages, num_words, media_omitted, links, sentiment_negative, sentiment_neutral, sentiment_positive = stats.fetchstats(
            selected_user, df)
        ...
        wedges, texts, autotexts = ax1.pie(sentiments, colors=colors, explode=explode, autopct='%1.1f%%',
