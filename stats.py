...
def fetchstats(selected_user, df):
    ...
    sentiment_positive = len(df[df['Sentiment'] > 0])
    sentiment_neutral = len(df[df['Sentiment'] == 0])
    sentiment_negative = len(df[df['Sentiment'] < 0])
    return num_messages, len(words), mediaommitted.shape[0], len(links), sentiment_negative, sentiment_neutral, sentiment_positive
